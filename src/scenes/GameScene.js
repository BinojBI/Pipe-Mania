export class GameScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.atlas(
      "pipesAtlas",
      "assets/images/pipes_and_cell_atlas.png",
      "assets/images/pipes_and_cell_atlas.json"
    );
    this.load.atlas(
      "uiElementAtlas",
      "assets/images/ui_element_atlas.png",
      "assets/images/ui_element_atlas.json"
    );
  ...
  createGrid() {
    ...
    for (let y = 0; y < this.rows; y++) {
      for (let x = 0; x < this.cols; x++) {
        ...
        cell.on("pointerover", () => {
          if (!this.gameOver) cell.setTint(0xffff00);
        });
        cell.on("pointerout", () => {
          cell.clearTint();
        });
    ...
    for (let dx = -1; dx <= 1; dx++) {
      for (let dy = -1; dy <= 1; dy++) {
        ...
        if (nx >= 0 && nx < this.cols && ny >= 0 && ny < this.rows) {
          neighbors.push(
            this.gridCells.find((c) => c.gridX === nx && c.gridY === ny)
          );
        }
      }
    }
  ...
  createUI() {
    ...
    this.delayText = this.add
      .text(this.gridStartX, this.gridStartY - 30, "Fill Start In: 0s", {
        font: "22px Arial",
        fill: "#ffffff",
      })
      .setOrigin(0, 0);
  ...
  createPipeQueue() {
    ...
    for (let i = 0; i < this.queueCount; i++) {
      ...
      const yPos =
        this.queueStartY + (this.queueCount - 1 - i) * this.pipeSpacing;

      const pipe = this.add
        .image(this.queueStartX, yPos, "pipesAtlas", randomPipe)
        .setOrigin(0.5)
        .setScale(this.cellSize / this.cellActualSize)
  ...
  placeStartPipe() {
    ...
    const validStartCells = this.gridCells.filter((cell) => {
      ...
      if (
        cell.gridX === 0 ||
        cell.gridX === this.cols - 1 ||
        cell.gridY === 0 ||
        cell.gridY === this.rows - 1
      )
        return false;
      const neighbors = this.getNeighbors(cell);
      if (neighbors.some((n) => n && this.gridData[n.gridY][n.gridX].isBlocked))
        return false;
      return true;
    });
    ...
    const startSprite = this.add
      .image(
        startCell.x + this.cellSize / 2,
        startCell.y + this.cellSize / 2,
        "pipesAtlas",
        "start_pipe"
      )
      .setOrigin(0.5)
  ...
  placePipeOnCell(cell, nextPipe) {
    ...
    const newAngle = Phaser.Utils.Array.GetRandom(
      this.validAnglesForType(newRandom)
    );
    const newPipe = this.add
      .image(this.queueStartX, this.queueStartY, "pipesAtlas", newRandom)
      .setOrigin(0.5)
      .setScale(this.cellSize / this.cellActualSize)
      .setAngle(newAngle);
  ...
  drawFill(x1, y1, x2, y2, callback) {
    ...
    const line = this.add
      .rectangle(
        x1,
        y1,
        this.fillThickness * w * 2.5,
        this.fillThickness * h,
        this.fillColor
      )
      .setOrigin(0, 0.5)
  ...
  animateCellFill(cellRef, angle, pipeType, fromDir = null, onComplete = null) {
    ...
    if (!data.isFilled && pipeType !== "start_pipe") {
      ...
      this.minimumRequiredLength = Math.max(
        this.minimumPipes - this.connectedPipesCount,
        0
      );
  ...
  handleNextPipe(gridX, gridY, angle, fromDir = null) {
    ...
    // Pick longest path among outgoing dirs (excluding fromDir)
    if (type === "cross_pipe") {
      ...
      if (!cellRef.savedBestDir) {
        ...
        if (cellRef.savedBestDir === chosen) {
          // Check cross pipe already visited and longest path already filled
    ...
    const validOutDirs = fromDir
      ? outDirs.filter((d) => d !== fromDir)
      : outDirs;
